<template>
  <div id="app">
    ...
    <TopAlbums v-if="showAlbums" :getAlbums="albums" @store-albums="albums.push($event)"/>
    {{ albums }}
    <div class="container">
      <Header />
...
export default {
  ...
  methods: {
    animate() {
      Array.from(document.querySelector('svg.scene').querySelectorAll('path')).forEach((path) => {
        setTimeout(() => {
          anime({
            targets: path,
