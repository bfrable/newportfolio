<template>
    <div class="now-playing-widget">
        <div class="now-playing-widget__container">
            ...
            <div class="now-playing-widget__icon"><a :href="track.url" target="_blank" class="now-playing-widget__link now-playing-widget__link--black"><i class="fab fa-lastfm"></i></a></div>

            <button @click="$emit('toggle-albums', console.log(true))">
              Add new number
            </button>
        </div>
    </div>          
  ...
  methods: {
    getCurrentlyPlaying() {
      axios.get('http://ws.audioscrobbler.com/2.0/?method=user.getrecenttracks&user=bfrable&api_key=c152ef462199b4430eec28a1727dc218&format=json&nowplaying=true', {
        ...
        this.nowPlaying = response.data.recenttracks.track[0]['@attr'].nowplaying;
      });
    },
  }
};
