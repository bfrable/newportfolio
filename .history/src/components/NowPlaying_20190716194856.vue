<template>
    <div class="now-playing-widget">
        <div class="now-playing-widget__container">
            ...
            <div @click="$parent.$parent.$emit('toggle-albums') && toggleAlbumsOpen()" v-bind:class="glow" class="now-playing-widget__icon"><a target="_blank" class="now-playing-widget__link now-playing-widget__link--black"><i class="fab fa-lastfm"></i></a></div>
            {{albumsOpen}}
        </div>
    </div>          
</template>
...
export default {
  ...
  computed: {
    glow: function() {
        return this.albumsOpen ? 'now-playing-widget__icon now-playing-widget__icon--glow' : 'now-playing-widget__icon';
    }
  },
