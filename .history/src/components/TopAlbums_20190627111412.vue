<template>
    <Carousel perPage="8" paginationColor="transparent" paginationActiveColor="#FFA797" class="albums">
        <Slide
...
export default {
  ...
  methods: {
    ...
    addRecentTracks(response) {
      this.tracks = response.data.recenttracks.track;
    },

    addTopArtists(response) {
    ...
    getTopArtists() {
      this.getURL('gettopartists', this.addTopArtists);
    },
    ...
    getTracks() {
      this.getURL('getrecenttracks', this.addRecentTracks);
    },
  },
};
</script>
